...
ROPEN_TOKEN_FILE="${ROPEN_TOKEN_FILE:-$HOME/.local/state/ropen-server.token}"
...
# Read ropen-server.py's shared auth token (0600, generated by the server on
# first run). Empty output if the file isn't there yet — caller appends the
# token only when non-empty.
open_core_ropen_token() {
  [[ -r "$ROPEN_TOKEN_FILE" ]] && cat "$ROPEN_TOKEN_FILE" 2>/dev/null
}
