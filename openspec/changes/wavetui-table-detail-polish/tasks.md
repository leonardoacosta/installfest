...
- [ ] [1.1] Change `Store.Snapshot()`'s item sort in `apps/wavetui/internal/store/store.go` [beads:if-nol8]
...
- [ ] [2.1] Render `🧿`/`📃` for `KindBead`/`KindProposal` in `apps/wavetui/internal/ui/ [beads:if-pzs7]
  ...
  - depends on: 1.1
- [ ] [2.2] Merge the `Created` column into the Item column as a leading `MM-dd ` prefix on the [beads:if-dob9]
  title in `queuepane.go`; drop the separate `Created` entry from `queueColumns`
  - depends on: 2.1
- [ ] [2.3] Add an overflow indicator to `queuepane.go`'s `View()` — when [beads:if-k6zr]
...
- [ ] [2.5] Thread `Root.layout()`'s already-computed `tableHeight` into `DetailPane` (new [beads:if-u48v]
  ...
  - depends on: 2.4
- [ ] [2.6] Change `apps/wavetui/internal/ui/root.go`'s `extraPaneReservedRows` constant from [beads:if-62p5]
...
- [ ] [2.7] Reorder `Root.View()` so `persistentExtras()` (Sessions/KPI/HeadlessBar) renders [beads:if-0w31]
...
- [ ] [3.1] `go test` for `apps/wavetui/internal/store/store.go`: confirm existing `Snapshot()` [beads:if-1n6r]
  ...
  - depends on: 1.1
- [ ] [3.2] Runtime-verify: build `apps/wavetui/cmd/wavetui`, run it in a real pty against a [beads:if-l6n2]
