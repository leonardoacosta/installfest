...
## Purpose
This capability owns `scripts/check.sh`, the repo's single verification command that
`scripts/hooks/pre-commit` blocks commits on. It defines what the gate SHALL check across every
layer of this dotfiles/tooling repo — shell scripts, chezmoi templates, Terraform, and (as of
this capability) the `apps/` layer's own test suites — so that a broken app cannot be committed
and auto-deployed (via `scripts/hooks/post-commit` / `wavetui-build.sh`) while its tests are red.
