...
- [x] [1.1] Add a `stripControlChars(s: string): string` helper to
...
- [x] [1.2] Apply `stripControlChars` at a single choke point covering both render paths.
  ...
  - depends on: 1.1
  - [type:security]
- [x] [1.3] Add/confirm `"test": "bun test"` in `apps/daily-brief/package.json`'s `scripts`
...
- [x] [2.1] Add `apps/daily-brief/test/plainRender.test.ts` (renders a snapshot fixture through
...
- [x] [2.2] Add `apps/daily-brief/test/docsState.test.ts` covering `src/sources/docsState.ts`'s
...
- [x] [2.3] Run `cd apps/daily-brief && bun test` (all pass, including the new
