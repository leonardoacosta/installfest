...
- [ ] [1.1] Add additive `Actor string` field to `timeline.Entry` in `apps/wavetui/internal/ [beads:if-1mtc]
...
- [ ] [2.1] Thread `interactionRecord.Actor` into the `Entry` returned by [beads:if-zjhw]
...
- [ ] [3.1] Update `renderTimelineEntry` in `apps/wavetui/internal/ui/memorytimelinepane.go` [beads:if-qm2j]
...
- [ ] [4.1] `go test` for `apps/wavetui/internal/timeline` and `apps/wavetui/internal/ui`: [beads:if-hgyi]
