...
- [ ] [1.1] In `scripts/lib/open-core.sh`'s `open_core_dispatch_browser`, base64-encode `$url`
...
- [ ] [1.2] In the same ssh invocation, change `-o StrictHostKeyChecking=no` to
  ...
  - depends on: 1.1
  - [type:security]
- [ ] [1.3] Grep for sibling `osascript` heredocs with interpolated dynamic values
...
- [ ] [2.1] Injection fixture: set `url='"; do shell script "touch /tmp/PWNED"; set x to "'`
  ...
  - depends on: 1.1
- [ ] [2.2] Run `scripts/check.sh` from the repo root (shellcheck at error severity) — exit 0.
