...
- [ ] [1.1] [P-1] `apps/cc-tmux/src/cc_tmux/cli.py`: add `_resolve_session_pane(window)` — returns
...
- [ ] [1.2] [P-1] `apps/cc-tmux/src/cc_tmux/cli.py`: swap `_build_session_bar`'s
...
- [ ] [1.3] [P-1] `apps/cc-tmux/src/cc_tmux/cli.py`: remove the `session_count` computation
...
- [ ] [1.4] [P-1] `apps/cc-tmux/src/cc_tmux/render.py`: remove `_session_glyph`,
...
- [ ] [1.5] [P-2] `openspec/specs/cc-tmux/spec.md`: apply this proposal's MODIFIED Requirement
...
- [ ] [2.1] [P-1] Extend `apps/cc-tmux/src/cc_tmux/testing.py`: self-test cases for
...
- [ ] [2.2] [P-1] Live verification: split a tmux window into two Claude panes (e.g. two different
...
- [ ] [2.3] [P-1] Live verification: focus a plain-shell pane in a window that also has a
...
- [ ] [2.4] [P-2] Live verification: confirm the `◉`/`◌` glyph no longer appears anywhere on row 2
