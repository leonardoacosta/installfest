...
- [ ] [1.1] [P-1] Confirm with the user, before writing `render.py` logic: the clarified mapping
...
- [ ] [1.2] [P-1] `apps/cc-tmux/src/cc_tmux/tmux.py`: add `OPT_SUBAGENT_FG = "@cc-subagent-fg"`
...
- [ ] [1.3] [P-1] `apps/cc-tmux/hooks/hooks.json`: add a `PreToolUse` entry matched on `"Task"`
...
- [ ] [2.1] [P-1] `apps/cc-tmux/src/cc_tmux/cli.py` (`cmd_register`): handle
...
- [ ] [2.2] [P-1] `apps/cc-tmux/src/cc_tmux/cli.py`: on a detected background dispatch, append the
...
- [ ] [2.3] [P-1] `apps/cc-tmux/src/cc_tmux/render.py` (`animated_icon` or a new wrapper): resolve
...
- [ ] [3.1] [P-2] If task 1.1 introduces new global options (e.g.
...
- [ ] [4.2] [P-1] Live verification, foreground: dispatch a real foreground sub-agent from a
...
- [ ] [4.3] [P-2] Live verification, background: dispatch a real background agent, observe the
