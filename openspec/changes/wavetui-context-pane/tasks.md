...
- [ ] [1.1] Add additive `CtxScanReport` Go types (project → class → document mirror of
...
- [ ] [2.1] Add `view-model` subcommand to `apps/ctx-scan/src/cli.ts`: `--root <path>`
...
- [ ] [2.2] Create `apps/wavetui/internal/sources/ctxscan.go`: `CtxScanSource` with
  ...
  - depends on: 1.1, 2.1
- [ ] [2.3] Add `apps/wavetui/internal/sources/ctxscan_test.go`: stub-CLI tests for decode of a
...
- [ ] [3.1] Create `apps/wavetui/internal/ui/ctxscanpane.go`: `ContextPane` implementing `Pane`
  ...
  - depends on: 1.1
- [ ] [3.2] Wire the tab in `apps/wavetui/internal/ui/root.go`: `tabScan` const, `[3] Context`
  ...
  - depends on: 3.1
- [ ] [3.3] Add `ctx_scan_poll_seconds` int knob (default 60) to
...
- [ ] [4.1] `go test ./...` in `apps/wavetui`: full suite passes including the new store,
  source, and pane tests; confirm existing store/root tests pass unmodified (additive-only
  check).
  - depends on: 2.3, 3.3
- [ ] [4.2] Runtime-verify: `bun test` in `apps/ctx-scan` passes; run `ctx-scan view-model
