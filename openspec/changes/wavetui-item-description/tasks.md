...
- [x] [1.1] Add additive `Description string` field to `store.Item` in `apps/wavetui/internal/ [beads:if-6rwu]
...
- [x] [2.1] Add `Description string \`json:"description"\`` to `beadRecord` in [beads:if-yijz]
  ...
  - depends on: 1.1
- [x] [2.2] Add a `summaryRe` regex to `apps/wavetui/internal/sources/openspec.go` extracting [beads:if-m66z]
...
- [x] [3.1] Render `Item.Description` (when non-empty) in `apps/wavetui/internal/ui/ [beads:if-jfhi]
...
- [x] [4.1] `go test` for all three touched packages: confirm existing tests pass unmodified, [beads:if-o9kb]
