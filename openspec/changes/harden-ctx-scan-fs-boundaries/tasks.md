...
- [ ] [1.1] Export `discovery.ts`'s existing `isWithin(child, root)` containment helper (already
...
- [ ] [1.2] Confine `discovery.ts`'s `walk`: before recursing into a child
  ...
  - depends on: 1.1
  [type:security]
- [ ] [1.3] Confine `imports.ts`'s `resolveImportChain`: add a `projectRoot` parameter and skip
  ...
  - depends on: 1.1
  [type:security]
- [ ] [1.4] Sharpen the `--probe-hooks` warning text in `cli.ts` (both the option help and the
...
- [ ] [2.1] Extend `apps/ctx-scan/test/discovery.test.ts` with a symlink-escape fixture case
  ...
  [type:test]
- [ ] [2.2] Run `cd apps/ctx-scan && bun test` — confirm all pass, including the new containment
