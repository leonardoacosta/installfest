...
| Headless children are long-running `claude -p` subprocesses; killing the daemon process (e.g. wavetui itself exits) could orphan them mid-turn | Out of scope for a first cut — `tasks.md` `[4.1]`'s E2E task must runtime-verify what actually happens to a child process group when the parent exits (documented finding, not assumed), and if orphaning is observed, file a follow-up bead rather than silently shipping unverified behavior. |
