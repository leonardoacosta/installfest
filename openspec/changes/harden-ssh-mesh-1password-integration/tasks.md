...
- [x] [4.2] Live-verify Req-3 on the real Mac: with the 1Password app running and the SSH agent enabled for the mesh key item (one-time manual toggle — see the User Gate task below), run an interactive `ssh homelab` (or an SSH-backed `git fetch`) and confirm a visible Touch ID/system-auth prompt fires. Then quit the 1Password app (or otherwise make the agent socket unavailable) and re-run the same command, confirming it falls back to the on-disk key with no error. — DONE 2026-07-21, after a real chain of discoveries:
  ...
  - depends on: 3.1
- [ ] [4.3] Live-verify Req-1 with a real (non-dry-run) rotation on a maintenance window: run `rotate-keys.sh` for real, confirm all peers stay reachable throughout (per the pre-existing lockout-safety guarantees from `harden-audit-remediation` Req-2, unchanged here), and confirm `op read` of `op://Personal/mesh-ssh` (corrected 2026-07-21 — see if-oudo — the vault is "Personal", not "Private", which does not exist on this account) on a scratch/test path returns the NEW key, not the rotated-out one. NOTE: as of commit deb5596, Phase 6b no longer attempts an automated 1Password write-back at all (confirmed unreliable, warns instead — see if-ooke) — the `op read` assertion in this task can only pass if the 1Password item is ALSO updated manually per that warning's instructions, immediately after this run. [beads:if-9pjw]
  - depends on: 1.1
