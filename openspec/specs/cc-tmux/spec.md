...
branch. The row's representative pane SHALL be resolved by preferring the window's tmux-focused
pane when that pane is tracked (carries a valid `@cc-state`), falling back to the existing
priority pick (waiting > idle > active, ties broken by tmux's iteration order) only when the
focused pane is untracked — so the row reflects the pane the user is actually looking at rather
than an arbitrary tie-break winner in a multi-pane window. The model letter SHALL be sourced from
the per-pane `session-context.<pane>.json` cache written by nexus-statusline — not from the
SessionStart hook payload, whose `model` field is unreliable and which never re-fires on a
mid-session `/model` switch. Right-justified on the same row, the plugin SHALL render Claude usage
statistics for the active nexus-agent credential: an account label, and SES:/5H:/7D: utilization
gauges (session-context %, 5-hour, and 7-day), each coloured by utilization threshold. This row
SHALL remain separate from the window-tabs row, whose own `status-right` stays usage-free.
...
- Then: the left side shows `F if > main` (model letter, project, branch) and the right side shows
  the account label plus SES:/5H:/7D: gauges

#### Scenario: the tmux-focused tracked pane is preferred over the priority pick
- Given: a window with two tracked Claude panes — pane A in project `oo` (state `idle`) and pane B
  in project `tc` (state `idle`), and the user has tmux-focused pane B
- When: the session-bar row renders
- Then: the row shows pane B's project/branch/model/usage identity, not pane A's, even though a
  priority-only pick (no `waiting` pane, tie broken by iteration order) would have picked pane A

#### Scenario: an untracked focused pane falls back to the priority pick
- Given: a window where the tmux-focused pane is a plain shell (no `@cc-state`) and a background
  pane is tracked and `waiting`
- When: the session-bar row renders
- Then: the row shows the background waiting pane's identity (fallback intact, not a blank row)
